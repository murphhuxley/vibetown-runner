...
describe('Duck', () => {
  ...
  it('climbs upward when the player is above on the same ladder route', () => {
    ...
    const duck = createDuck(0, { x: 10, y: groundedY });
    const moved = moveDuckToward(duck, grid, { x: 10, y: groundedY - 3 }, new Set(), 0);
  ...
  it('faces toward the player while descending a ladder', () => {
    ...
    const moved = moveDuckToward(duck, grid, { x: 14, y: groundedY }, new Set(), 0);
  ...
  it('hesitates when hesitation chance is 1', () => {
    ...
    const duck = createDuck(0, { x: 10, y: groundedY });
    const playerPos = { x: 5, y: groundedY };
    const moved = moveDuckToward(duck, grid, playerPos, new Set(), 1.0);
